...
Date: 2026-09-17 · Status: **accepted** · [#782](https://github.com/scttfrdmn/burroughs/issues/782) · Recon [#781](https://github.com/scttfrdmn/burroughs/issues/781) · Opens the real-threads fork (contract §§2–6)

**Stamped 2026-09-17 (Scott, on [#782](https://github.com/scttfrdmn/burroughs/issues/782#issuecomment-5717612111) — the ruled path: "Stamp ADR 0087 now … it records the state honestly, and leaving the charter unstamped doesn't preserve optionality").** `Status: accepted` cites that ruling — behaviour 3: an ADR's `Status` is a citation to an approval. This ADR opens the tier in the same form [#688 → ADR 0084](0084-the-component-loader-and-canonical-abi-lift-lower-for-value-types-sync-only-behind-gate-components.md) opened the component tier and [#735 → ADR 0086](0086-the-async-canonical-abi-behind-gate-async-opens-phase-3.md) opened the async tier. Drafted by the actor from committed sources (see *Sources*); durability is not independence, so this ADR stays `Ratio-Class: carried`. Opening the tier is not a gate flip and lands no engine or fork code (see *Consequences*); the chair's ruling sequences the next artifact as a **second async guest** (Phase 3's async-lift half, #771), the base decision (#782 D-a) **held**, before this charter's first slice can start.
...
A `GEOEXPERIMENT`-gated real-threads experiment over a p3-async base, in the fork's own Go tree — a **private tree, separate from the Burroughs repo** (register §3) — not a stock `GOOS=wasip3` claim. **wasmpreempt is the precedent for the host-agnostic half only** (the back-edge poll codegen, the shared-memory safepoint word, the atomics lowerings, passive segments, the STW broadcast mechanism — all validated on two engines/ISAs). Its scheduler-liveness half is **JS-coupled and is not ported** — and re-reading it against the corrected D-6 (#782) shows it has three fates, not one. Part **disappears**: the m0 heartbeat's `Atomics.waitAsync` async-park and the `setTimeout` bell exist so a *single* JS event-loop thread never blocks synchronously, and Burroughs — goroutine-per-agent with real blocking excursions (§5 H-1/H-4) — has no sole thread to protect, so an M blocks like any other and both mechanisms are unnecessary rather than reimplemented (the native m0 design, a deletion). Part **binds** to a Burroughs counterpart: `newosproc` → `Spawn`, futex → the 0xFE region, GC-STW trigger → `Stop`, `netpoll`/blocking → `waitable-set.wait`. Part **transfers as code**: the back-edge poll, the atomics lowerings, the STW broadcast mechanism, passive segments. The result is a liveness delta **smaller** than wasmpreempt's, not a re-port of it — which is why the reversal trigger stays D-1 flipping to yes, and not the seam retirement. The reversal trigger for this whole shape is **D-1 flipping to yes** (a `thread.spawn` in the shipped ABI, giving an independent engine), which is on no upstream horizon (#781 A3).
