...
func asyncLowerFunc(impl asyncLowerImpl, hasResult bool, table *subtaskTable) interp.CanonFunc {
	return func(c *interp.CanonCaller, args []interp.Value) ([]interp.Value, error) {
		...
		onResolve := func(v canon.Value) {
			// Lower the result to the retptr on resolution (the only guest-memory write), then mark
			// RETURNED. For the blocking arm this fires later, off the waitable-set loop (2a-i-B-2); a
			// blocked callee that never calls onResolve leaves the retptr untouched.
			if hasResult && resolveErr == nil {
		...
		if st.resolved {
			// Sync-resolving arm (2a-i-A): the result is already at the retptr; return [RETURNED].
			return []interp.Value{interp.I32(int32(subtaskReturned))}, nil
		}
		// Blocking arm (2a-i-B): the callee started but did not resolve inline. Register the subtask and
		// return [state | (subtaski<<4)]. The guest observes the resolution via the waitable-set loop
		// (2a-i-B-2); a guest that async-lowers-blocking AND uses waitable-set built-ins is still refused
		// at bind by gateAsync until that loop lands.
		subtaski := table.add(st)
		return []interp.Value{interp.I32(packSubtaskWait(st.state, subtaski))}, nil
	}
}
